-- Migration v13: Fee structure (fee policy)
-- Run ONCE in Supabase Dashboard -> SQL Editor. Idempotent — safe to run again.
-- Adds fee_plans (payment options stored per course) and updates base fee columns.
--
...
-- 1) New column (the website falls back to its built-in fee plans until this exists)
